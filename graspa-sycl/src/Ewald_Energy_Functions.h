...
void Initialize_Vectors(Boxsize Box, size_t Oldsize, size_t Newsize, Atoms Old,
                        size_t numberOfAtoms, int3 kmax, const sycl::nd_item<3> &item_ct1)
{
  int kx_max = kmax.x();
  int ky_max = kmax.y();
  int kz_max = kmax.z();
  ...
  for(size_t posi=0; posi < Oldsize; ++posi)
  {
    ...
    s *= 2 * M_PI;
    tempcomplex.real = sycl::cos(s.x()); tempcomplex.imag = sycl::sin(s.x());
        Box.eik_x[posi + 1 * numberOfAtoms] = tempcomplex;
    tempcomplex.real = sycl::cos(s.y()); tempcomplex.imag = sycl::sin(s.y());
        Box.eik_y[posi + 1 * numberOfAtoms] = tempcomplex;
    tempcomplex.real = sycl::cos(s.z()); tempcomplex.imag = sycl::sin(s.z());
...
void Initialize_EwaldVector_General(Boxsize Box, int3 kmax, Atoms *d_a,
                                    Atoms New, Atoms Old, size_t Oldsize,
                                    size_t Newsize, size_t SelectedComponent,
                                    size_t Location, size_t chainsize,
                                    size_t numberOfAtoms, int MoveType, const sycl::nd_item<3> &item_ct1)
{
  ...
  Initialize_Vectors(Box, Oldsize, Newsize, Old, numberOfAtoms, kmax, item_ct1);
}

void Initialize_EwaldVector_LambdaChange(Boxsize Box, int3 kmax,
                                         Atoms *d_a, Atoms Old, size_t Oldsize,
                                         double2 newScale, const sycl::nd_item<3> &item_ct1)
{
  Initialize_Vectors(Box, Oldsize, 0, Old, Oldsize, kmax, item_ct1);
}

void Initialize_EwaldVector_Reinsertion(Boxsize Box, int3 kmax,
                                        double3 *temp, Atoms *d_a,
                                        Atoms Old, size_t Oldsize,
                                        size_t Newsize, size_t realpos,
                                        size_t numberOfAtoms,
                                        size_t SelectedComponent, const sycl::nd_item<3> &item_ct1)
{
  for(size_t i = 0; i < Oldsize; i++)
  {
...
void Initialize_EwaldVector_IdentitySwap(
    Boxsize Box, int3 kmax, double3 *temp, Atoms *d_a, Atoms Old,
    size_t Oldsize, size_t Newsize, size_t realpos, size_t numberOfAtoms,
    size_t OLDComponent, size_t NEWComponent, const sycl::nd_item<3> &item_ct1)
{
  for(size_t i = 0; i < Oldsize; i++)
  {
...
void JustStore_Ewald(Boxsize Box, size_t nvec, const sycl::nd_item<3> &item_ct1)
{
...
void Fourier_Ewald_Diff(Boxsize Box, Atoms Old, double alpha_squared,
                        double prefactor, int3 kmax, double recip_cutoff,
                        ...
                        const sycl::nd_item<3> &item_ct1, uint8_t *dpct_local)
{
  ...
  size_t kxyz = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
                item_ct1.get_local_id(2);
  int cache_id = item_ct1.get_local_id(2);
  size_t i_within_block =
      kxyz -
      item_ct1.get_group(2) *
          item_ct1.get_local_range(
              2); // for recording the position of the thread within a block
  double tempE = 0.0;
  ...
  if (item_ct1.get_group(2) >= Nblock) kxyz -=
  ...
  if(kxyz < nvec)
  {
    ...
    {
      double3 ax; ax.x() = Box.InverseCell[0];
          ax.y() = Box.InverseCell[3]; ax.z() = Box.InverseCell[6];
      double3 ay; ay.x() = Box.InverseCell[1];
          ay.y() = Box.InverseCell[4]; ay.z() = Box.InverseCell[7];
      double3 az; az.x() = Box.InverseCell[2];
          az.y() = Box.InverseCell[5]; az.z() = Box.InverseCell[8];
      ...
      if (item_ct1.get_group(2) < Nblock) // Guest-Guest, do the normal Ewald//
      {
        if(UseTempVector)
        {
  ...
  */
  item_ct1.barrier();
  //Partial block sum//
  int i = item_ct1.get_local_range(2) / 2;
  while(i != 0)
  {
  ...
  if (cache_id == 0) { Blocksum[item_ct1.get_group(2)] = sdata[0]; }
}
  ...
  */
  dpct::get_default_queue().parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) {
        JustStore_Ewald(Box, numberOfWaveVectors, item_ct1);
      });
}
                         ...
                         const sycl::nd_item<3> &item_ct1)
{
  size_t i = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
             item_ct1.get_local_id(2);
  if(i < nvec) storedEik[i] = totalEik[i];
}
    ...
    */
    dpct::get_default_queue().parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, Nblock) *
                          ...
                          sycl::range<3>(1, 1, Nthread)),
        [=](sycl::nd_item<3> item_ct1) {
          Update_Ewald_Stored(Box.storedEik, Box.totalEik, numberOfWaveVectors, item_ct1);
        });
  }
}
...
double GPU_EwaldDifference_General(Boxsize &Box, Atoms *&d_a, Atoms &New,
                                   ...
                                   size_t Location, double2 newScale)
{
  ...
  q_ct1.parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) {
        Initialize_EwaldVector_General(Box, Box.kmax, d_a, New, Old, Oldsize, Newsize, SelectedComponent,
                                       Location, chainsize, numberOfAtoms, MoveType, item_ct1);
      });
  ...
  */
  ...
  q_ct1.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, Nblock * 2) *
                                       ...
                                       sycl::range<3>(1, 1, Nthread)),
                     [=](sycl::nd_item<3> item_ct1) {
                       Fourier_Ewald_Diff(Box, Old, alpha_squared, prefactor, Box.kmax,
                                          Box.ReciprocalCutOff, Oldsize, Newsize, Blocksum,
                                          UseTempVector, Nblock, item_ct1, dpct_local_acc_ct1.get_pointer());
                     });
  });

  double sum[Nblock * 2]; double tot = 0.0; double HG_tot = 0.0;
  q_ct1.memcpy(sum, Blocksum, 2 * Nblock * sizeof(double))
      .wait(); // HG + GG Energies//
                                       ...
                                       size_t UpdateLocation)
{
  ...
  q_ct1.parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) {
        Initialize_EwaldVector_Reinsertion(Box, Box.kmax, temp, d_a, Old, Oldsize, Newsize, UpdateLocation,
                                          numberOfAtoms, SelectedComponent, item_ct1);
      });
  ...
  */
  ...
  q_ct1.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, Nblock * 2) *
                     ...
                     [=](sycl::nd_item<3> item_ct1) {
                       Fourier_Ewald_Diff(Box, Old, alpha_squared, prefactor, Box.kmax,
                                          Box.ReciprocalCutOff, Oldsize, Newsize, Blocksum, false,
                                          Nblock, item_ct1, dpct_local_acc_ct1.get_pointer());
                     });
  });
  double sum[Nblock * 2]; double tot = 0.0;  double HG_tot = 0.0;
  q_ct1.memcpy(sum, Blocksum, 2 * Nblock * sizeof(double)).wait();
  for(size_t i = 0; i < Nblock; i++){tot += sum[i];}
  for(size_t i = Nblock; i < 2 * Nblock; i++){HG_tot += sum[i];}
  SystemComponents.tempdeltaHGEwald = 2.0 * HG_tot;
  return tot;
    ...
    size_t NEWComponent, size_t UpdateLocation)
{
  ...
  q_ct1.parallel_for(
      sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
      [=](sycl::nd_item<3> item_ct1) {
        Initialize_EwaldVector_IdentitySwap(Box, Box.kmax, temp, d_a, Old, Oldsize, Newsize, UpdateLocation,
                                            numberOfAtoms, OLDComponent, NEWComponent, item_ct1);
      });
                                     ...
                                     uint8_t *dpct_local)
{
  ...
  if(kxyz < nvec)
  {
    ...
    if((ksqr != 0) && (static_cast<double>(ksqr) < recip_cutoff))
    {
      double3 ax = {Box.InverseCell[0], Box.InverseCell[3],
                          Box.InverseCell[6]};
      double3 ay = {Box.InverseCell[1], Box.InverseCell[4],
                          Box.InverseCell[7]};
      double3 az = {Box.InverseCell[2], Box.InverseCell[5],
                          Box.InverseCell[8]};
      ...
      double3 kvec_x = ax * scalarkx;
      double3 kvec_y = ay * scalarky;
      double3 kvec_z = az * scalarkz;
      double factor = (kx == 0) ? (1.0 * prefactor) : (2.0 * prefactor);
                                        ...
                                        double2 newScale, int MoveType)
{
  ...
  q_ct1.parallel_for(
      sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
      [=](sycl::nd_item<3> item_ct1) {
        Initialize_EwaldVector_LambdaChange(Box, Box.kmax, d_a, Old, Oldsize, newScale, item_ct1);
      });
  ...
  */
  ...
  sycl::local_accessor<uint8_t, 1> dpct_local_acc_ct1(
      sycl::range<1>(Nthread * sizeof(double)), cgh);
  q_ct1.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, Nblock) *
                                           sycl::range<3>(1, 1, Nthread),
                                       sycl::range<3>(1, 1, Nthread)),
                     [=](sycl::nd_item<3> item_ct1) {
                       Fourier_Ewald_Diff_LambdaChange(Box, Old, alpha_squared, prefactor, Box.kmax,
                                                       Box.ReciprocalCutOff, Oldsize, Blocksum, UseTempVector,
                                                       newScale.y(), item_ct1, dpct_local_acc_ct1.get_pointer());
                     });
  });
  double Host_sum[Nblock]; double tot = 0.0;
  q_ct1.memcpy(Host_sum, Blocksum, Nblock * sizeof(double)).wait();
                        ...
                        const sycl::nd_item<3> &item_ct1)
{
  ...
  size_t i = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
             item_ct1.get_local_id(
                 2); // number of threads = number of atoms in the system
  if(i < numberOfAtoms)
  {
    //size_t atom_i = i;
    size_t atom_i = i;
...
void TotalEwald(Atoms* d_a, Boxsize Box, double* BlockSum, Complex* eik_x, Complex* eik_y, Complex* eik_z, Complex* Eik, size_t totAtom, size_t Ncomp, size_t NAtomPerThread, size_t residueAtoms,
                const sycl::nd_item<3> &item_ct1, uint8_t *dpct_local)
{
  auto sdata = (double3 *)dpct_local; // shared memory for partial sum//
  // maybe just need Complex//
  int cache_id = item_ct1.get_local_id(2);
  size_t total_ij = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
                    item_ct1.get_local_id(2);
  size_t ij_within_block =
      total_ij - item_ct1.get_group(2) * item_ct1.get_local_range(2);

  size_t kxyz = item_ct1.get_group(2); // Each block takes over one grid point//
  ...
  //double3 kvec_z = az * 2.0 * M_PI * (double) kz;
  double factor = (kx == 0) ? (1.0 * prefactor) : (2.0 * prefactor);
  double3 t1  = static_cast<double3>(kvec_x);
  double3 t2  = static_cast<double3>(kvec_y);
  double3 t3  = static_cast<double3>(kvec_z);
  double3 tempkvec  = t1 + t2 + t3;
  //double3 tempkvec  = static_cast<double3>(kvec_x) + static_cast<double3>(kvec_y) + static_cast<double3>(kvec_z);
  //double3 tempkvec  = static_cast<double3>(kvec_x) + static_cast<double3>(kvec_y) + static_cast<double3>(kvec_z);
  double  rksq      = dot(tempkvec, tempkvec);
  double  temp      = 0.0;
  ...
  if((ksqr != 0) && ((double)(ksqr) < Box.ReciprocalCutOff))
  {
    ...
    if(residueAtoms > 0)
    {
      ...
      //The thread from zero to residueAtoms will each take another atom//
      {
        ...
        size_t Atom =
            item_ct1.get_local_range(2) * NAtomPerThread + ij_within_block;
        size_t AbsoluteAtom = Atom; size_t comp = 0; size_t totsize = 0;
        for(size_t it_comp = 0; it_comp < Ncomp; it_comp++)
        {
          size_t Atom_ijk = d_a[it_comp].size;
  ...
  */
  ...
  if(cache_id == 0)
  {
    ...
    BlockSum[item_ct1.get_group(2)] = temp * NORM;
    Eik[item_ct1.get_group(2)].real = sdata[0].x();
    Eik[item_ct1.get_group(2)].imag = sdata[0].y();
  }
}
...
MoveEnergy Ewald_TotalEnergy(Simulations& Sim, Components& SystemComponents, bool UseOffSet)
{
  ...
  if(NTotalAtom > 0)
  {
    ...
    Complex *eikx; eikx = sycl::malloc_device<Complex>(NTotalAtom * (Box.kmax.x() + 1) * sizeof(Complex), q_ct1);
    Complex *eiky; eikx = sycl::malloc_device<Complex>(NTotalAtom * (Box.kmax.y() + 1) * sizeof(Complex), q_ct1);
    Complex *eikz; eikx = sycl::malloc_device<Complex>(NTotalAtom * (Box.kmax.z() + 1) * sizeof(Complex), q_ct1);
    /*
    ...
    */
    ...
    q_ct1.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, Nblock) *
                                         ...
                                         sycl::range<3>(1, 1, Nthread)),
                       [=](sycl::nd_item<3> item_ct1) {
                         Setup_Ewald_Vector(Box, eikx, eiky, eikz, d_a, NTotalAtom,
                          Total_Components, UseOffSet, item_ct1);
                       });
    ...
    if(Nblock > Sim.Nblocks)
    {
      ...
      (Sim.Blocksum) =
          (typename std::remove_reference<decltype(Sim.Blocksum)>::type)
              sycl::malloc_device(Nblock * sizeof(double), q_ct1);
    }
    Nthread= 128;
    ...
    */
    q_ct1.submit([&](sycl::handler &cgh) {
    sycl::local_accessor<uint8_t, 1> dpct_local_acc_ct1(
      sycl::range<1>(Nthread * sizeof(double3)), cgh);
    q_ct1.parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, Nblock) *
                              sycl::range<3>(1, 1, Nthread),
                          sycl::range<3>(1, 1, Nthread)),
        [=](sycl::nd_item<3> item_ct1) {
          TotalEwald(d_a, Box, Sim.Blocksum, eikx, eiky, eikz, Box.totalEik, NTotalAtom,
                     Total_Components, NAtomPerThread, residueAtoms, item_ct1, dpct_local_acc_ct1.get_pointer());
        });
    });

    sycl::free(eikx, q_ct1); sycl::free(eiky, q_ct1); sycl::free(eikz, q_ct1);

    double HostTotEwald[Nblock];
    q_ct1.memcpy(HostTotEwald, Sim.Blocksum, Nblock * sizeof(double)).wait();
