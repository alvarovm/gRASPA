...
struct Move_Statistics
{
  ...
  //Translation Move//
  size_t TranslationAccepted;
  size_t TranslationTotal;
  double TranslationAccRatio;
  //Rotation Move//
  size_t RotationAccepted;
  size_t RotationTotal;
...
struct Components
{
  ...
  int3 NComponents; // Total components (x), Framework Components (y),
                          // Guest Components (z)
...
struct Simulations //For multiple simulations//
{
  ...
  int2 *ExcludeList; // Atoms to exclude during energy calculations: x:
                           // component, y: molecule-ID (may need to add z and
                           // make it int3, z: atom-ID)
