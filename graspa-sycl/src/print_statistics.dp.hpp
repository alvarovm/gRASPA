...
static inline void Print_Translation_Statistics(Move_Statistics MoveStats,
                                                double3 MaxTranslation)
{
  ...
  printf("Max Translation: %.10f, %.10f, %.10f\n", MaxTranslation.x(),
         MaxTranslation.y(), MaxTranslation.z());
  printf("===========================================================\n");
}

static inline void Print_Rotation_Statistics(Move_Statistics MoveStats,
                                             double3 MaxRotation)
{
  ...
  printf("Max Rotation: %.10f, %.10f, %.10f\n", MaxRotation.x(),
         MaxRotation.y(), MaxRotation.z());
  printf("===========================================================\n");
}

static inline void ComputeFugacity(double &AverageWr, double &AverageMu,
                                   double &Fugacity, Boxsize &Box,
                                   Components &SystemComponents,
                                   double3 Stats, Units Constants)
{
...
static inline void Print_Widom_Statistics(Components& SystemComponents, Boxsize Box, Units& Constants, size_t comp)
{
  ...
  double RosenError =
      2.0 *
      pow((AvgBlockRosen.y() - AvgBlockRosen.x() * AvgBlockRosen.x()), 0.5);

  double2 AvgBlockMu;
  AvgBlockMu.x() = totMu.x() / (double)SystemComponents.Nblock;
  AvgBlockMu.y() = totMu.y() / (double)SystemComponents.Nblock;
  double MuError =
      2.0 * pow((AvgBlockMu.y() - AvgBlockMu.x() * AvgBlockMu.x()), 0.5);
  ...
  double FugaError =
      2.0 * pow((AvgBlockFuga.y() - AvgBlockFuga.x() * AvgBlockFuga.x()), 0.5);
  printf("=========================AVERAGE========================\n");
