...
void read_simulation_input(bool *UseGPUReduction, bool *Useflag, bool *noCharges, int *InitializationCycles, int *EquilibrationCycles, int *ProductionCycles, size_t *Widom_Trial, size_t *Widom_Orientation, double *Pressure, double *Temperature, size_t *AllocateSize, bool *ReadRestart, int *RANDOMSEED, bool *SameFrameworkEverySimulation, size_t& NumberOfComponents)
{
  ...
  *SameFrameworkEverySimulation = tempSameFrameworkEverySimulation;
  NumberOfComponents += tempNComp; //NumberOfComponents starts with 1 (framework)//
...
void CheckFrameworkCIF(Boxsize& Box, Atoms& Framework, PseudoAtomDefinitions& PseudoAtom, std::string& Frameworkfile, bool UseChargeFromCIF, double3 NumberUnitCells, Components& SystemComponents)
{
  ...
  int2 atom_site_occurance = {0, 0}; int count=0;
  int atomsiteCount = 0;
  ...
  //Loop Over the Atoms//
  double3 pos; size_t Type; double Charge = 0.0;
  //size_t AtomID = 0;
    ...
    for(size_t ix = 0; ix < NumberUnitCells.x(); ix++)
      for(size_t jy = 0; jy < NumberUnitCells.y(); jy++)
        for(size_t kz = 0; kz < NumberUnitCells.z(); kz++)
        {
          ...
          pos.x() = super_fx*Box.Cell[0]+super_fy*Box.Cell[3]+super_fz*Box.Cell[6];
          pos.y() = super_fx*Box.Cell[1]+super_fy*Box.Cell[4]+super_fz*Box.Cell[7];
          pos.z() = super_fx*Box.Cell[2]+super_fy*Box.Cell[5]+super_fz*Box.Cell[8];
          super_pos.push_back(pos);
  ...
  Framework.pos       = (double3*) malloc(super_pos.size() * sizeof(double3));
  Framework.scale     = (double*)  malloc(super_pos.size()   * sizeof(double));
  Framework.charge    = (double*)  malloc(super_pos.size()   * sizeof(double));
  Framework.scaleCoul = (double*)  malloc(super_pos.size()   * sizeof(double));
  Framework.Type      = (size_t*)  malloc(super_pos.size()   * sizeof(size_t));
  Framework.MolID     = (size_t*)  malloc(super_pos.size()   * sizeof(size_t));

  bool FrameworkHasPartialCharge = false; double ChargeSum = 0.0;
  for(size_t i = 0; i < super_pos.size(); i++)
  {
    Framework.pos[i] = super_pos[i]; 
    ...
    ChargeSum += std::abs(Framework.charge[i]);
  }
  if(ChargeSum > 1e-6) FrameworkHasPartialCharge = true;
  Framework.size = super_pos.size(); Framework.Molsize = super_pos.size(); Framework.Allocate_size = Framework.size;
...
void POSCARParser(Boxsize& Box, Atoms& Framework, PseudoAtomDefinitions& PseudoAtom, std::string& poscarfile)
{
  ...
  std::vector<double>SCALE; std::vector<double>CHARGE; std::vector<double>SCALECOUL;
  ...
  while (std::getline(file, str))
  {
    if(linenum > 7) //Loop over all the atoms
    {
      ...
      if(!cartesian)
      {
        ...
        POS.push_back({fx*Cell[0]+fy*Cell[3]+fz*Cell[6], fx*Cell[1]+fy*Cell[4]+fz*Cell[7], fx*Cell[2]+fy*Cell[5]+fz*Cell[8]}); 
      }else
      {
        POS.push_back({std::stod(termsScannedLined[0]), std::stod(termsScannedLined[1]), std::stod(termsScannedLined[2])});
      } 
  ...
  Box.Cell = convert1DVectortoArray(Cell);
  //read the data into the framework struct//
  Framework.pos       = convert1DVectortoArray(POS);
  Framework.scale     = convert1DVectortoArray(SCALE);
  Framework.charge    = convert1DVectortoArray(CHARGE);
  Framework.scaleCoul = convert1DVectortoArray(SCALECOUL);

  Framework.Type      = convert1DVectortoArray(TYPE);
  Framework.MolID     = convert1DVectortoArray(MOLID);
  Framework.Allocate_size = counter; 
  Framework.size = counter;
...
void read_component_values_from_simulation_input(Components& SystemComponents, Move_Statistics& MoveStats, size_t AdsorbateComponent, Atoms& Mol, PseudoAtomDefinitions PseudoAtom, size_t Allocate_space)
{
  ...
  double TranslationProb=0.0; double RotationProb=0.0; double WidomProb=0.0; double ReinsertionProb = 0.0; double IdentitySwapProb = 0.0; double SwapProb = 0.0; double CBCFProb = 0.0; double TotalProb=0.0;
  ...
  while (std::getline(file, str))
  {
    ...
    if(counter >= start_counter) //start reading after touching the starting line number
    {
      ...
      {
        termsScannedLined = split(str, ' ');
        IdentitySwapProb=std::stod(termsScannedLined[1]);
      ...
      { 
        for(size_t b = 0; b < Mol.Molsize; b++) SystemComponents.ConsiderThisAdsorbateAtom_HOST.push_back(false);
        ...
        if (str.find("DNNPseudoAtoms", 0) != std::string::npos)
        {
          ...
          for(size_t a = 1; a < termsScannedLined.size(); a++)
          {
            ...
            for(size_t b = 0; b < Mol.Molsize; b++)
            {
              if(Type == Mol.Type[b]) 
              {
                SystemComponents.ConsiderThisAdsorbateAtom_HOST[b] = true;
                printf("AtomName: %s, Type: %zu, b: %zu, Consider: %d\n", AtomName.c_str(), Type, b, SystemComponents.ConsiderThisAdsorbateAtom_HOST[b]);
              }
            }
          }
        }
  ...
  if(SystemComponents.Moleculesize[currentCompsize-1] == 1 && RotationProb > 1e-10)
  {
  ...
  MoveStats.TranslationProb=TranslationProb; MoveStats.RotationProb=RotationProb; MoveStats.WidomProb=WidomProb; MoveStats.ReinsertionProb=ReinsertionProb; 
  MoveStats.IdentitySwapProb= IdentitySwapProb;
  MoveStats.CBCFProb=CBCFProb; MoveStats.SwapProb=SwapProb; 
  MoveStats.TotalProb=TotalProb;
